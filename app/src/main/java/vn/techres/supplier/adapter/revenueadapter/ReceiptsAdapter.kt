...
class ReceiptsAdapter(var context: Context) :
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        with(holder) {
            ...
            binding.nameCreate.ellipsize = TextUtils.TruncateAt.MARQUEE
            binding.nameCreate.marqueeRepeatLimit = -1
            binding.nameCreate.isSelected = true
            when (dataList.object_type) {
                0 -> {
                    binding.objectType.text = "Khác"
                    binding.nameCreate.text = dataList.supplier_employee_create_full_name
                }
                1 -> {
                    binding.objectType.text = "Đơn hàng"
                    binding.nameCreate.text = dataList.restaurant_name
                }
                2 -> {
                    binding.objectType.text = "Phiếu nhập kho"
                    binding.nameCreate.text = dataList.restaurant_name
                }
            }
