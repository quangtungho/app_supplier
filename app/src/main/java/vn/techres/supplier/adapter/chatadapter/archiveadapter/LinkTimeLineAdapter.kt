...
class LinkTimeLineAdapter(var context: Context) :
    RecyclerView.Adapter<LinkTimeLineAdapter.ViewHolder>() {
    private var linkGroupAdapter: LinkGroupAdapter? = null
    ...
    private fun setAdapterImageVideo(recyclerView: RecyclerView, listItem: ArrayList<ListFile>) {
        linkGroupAdapter = LinkGroupAdapter(context)
        linkGroupAdapter!!.setDataSource(listItem)
        recyclerView.layoutManager =
            LinearLayoutManager(SupplierApplication.context)
        recyclerView.adapter = linkGroupAdapter
    }
}