...
import vn.techres.supplier.adapter.chatadapter.archiveadapter.ImageTimeLineAdapter
 ...
 * ================================================
 */
class MediaGroupFragment : Fragment() {
    var binding: FragmentMediaGroupBinding? = null
    var adapter: ImageTimeLineAdapter? = null
    ...
    private fun onBody() {
        ...
        adapter = context?.let { ImageTimeLineAdapter(it) }
        binding!!.recyclerView.adapter = adapter
        binding!!.recyclerView.setHasFixedSize(true)
    }
    /**
     * goi API get danh sách group media chat
     */
    private fun getMediaGroup() {
        val params = BaseParams()
        params.http_method = 0
        params.project_id = AppConfig.PROJECT_CHAT
        params.request_url =
            "/api/message-tms-supplier/get-message-file?limit=2&page=0&group_id=$groupId&type=${TechresEnum.MEDIA_GROUP}"
                ...
                .subscribe(object : Observer<ArchiveByGroupResponse> {
                    ...
                    override fun onNext(response: ArchiveByGroupResponse) {
                        if (response.status == AppConfig.SUCCESS_CODE) {
                            ...
                            Toast.makeText(SupplierApplication.context,
                                response.message,
