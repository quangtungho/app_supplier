...
import vn.techres.supplier.adapter.chatadapter.archiveadapter.FileTimeLineAdapter
...
class FileFragment : Fragment() {
    ...
    var adapter: FileTimeLineAdapter? = null
    ...
    private fun onBody() {
        ...
        adapter = context?.let { FileTimeLineAdapter(it) }
        binding!!.recyclerView.adapter = adapter
        binding!!.recyclerView.setHasFixedSize(true)
    }
    /**
     * goi API get danh sách group file chat
     */
    private fun getFileGroup() {
        val params = BaseParams()
        params.http_method = 0
        params.project_id = AppConfig.PROJECT_CHAT
        params.request_url =
            "/api/message-tms-supplier/get-message-file?limit=2&page=0&group_id=$groupId&type=${TechresEnum.FILE_GROUP}"
