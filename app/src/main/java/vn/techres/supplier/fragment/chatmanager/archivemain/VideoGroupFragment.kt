...
import vn.techres.supplier.adapter.chatadapter.archiveadapter.VideoTimeLineAdapter
 ...
 * ================================================
 */
class VideoGroupFragment : Fragment() {
    var binding: FragmentMediaGroupBinding? = null
    var adapter: VideoTimeLineAdapter? = null
    ...
    private fun onBody() {
        ...
        adapter = context?.let { VideoTimeLineAdapter(it) }
        binding!!.recyclerView.adapter = adapter
        binding!!.recyclerView.setHasFixedSize(true)
    }
    /**
     * goi API get danh sách group video chat
     */
    private fun getVideoGroup() {
        val params = BaseParams()
        params.http_method = 0
        params.project_id = AppConfig.PROJECT_CHAT
        params.request_url =
            "/api/message-tms-supplier/get-message-file?limit=2&page=0&group_id=$groupId&type=${TechresEnum.VIDEO_GROUP}"
