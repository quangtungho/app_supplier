...
import vn.techres.supplier.adapter.ordermanageradapterr.supplierorderadapter.DialogBillMaterialOrderDeliveringAdapter
 ...
 * ================================================
 */
class OrderDeliveryDetailFragment : BaseBindingFragment<FragmentOrderDetailBinding>(
    FragmentOrderDetailBinding::inflate
) {
    val tagName: String = OrderDeliveryDetailFragment::class.java.name
    private var adapter: DialogBillMaterialOrderDeliveringAdapter? = null
    ...
    fun setAdapter() {
        adapter = DialogBillMaterialOrderDeliveringAdapter(mainActivity!!.baseContext)
            ...
            .subscribe(object : Observer<DetailOrderBillAllResponse> {
                ...
                override fun onNext(response: DetailOrderBillAllResponse) {
                    if (response.status == AppConfig.SUCCESS_CODE) {
                        ...
                        binding!!.totalmaterial.text = formatFloatToString(
                            dataDetailBillOrder.total_material!!.toFloat()
                        )
                        binding!!.totalMoney2.text =
                            Currency.formatCurrencyDecimal(
                                dataDetailBillOrder.total_amount_reality.toFloat()
                            )
                    } else {
                        Toast.makeText(context, response.message, Toast.LENGTH_LONG).show()
                    }
