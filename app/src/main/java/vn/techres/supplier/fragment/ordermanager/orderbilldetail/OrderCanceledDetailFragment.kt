...
import vn.techres.supplier.adapter.ordermanageradapterr.supplierorderadapter.DialogBillMaterialOrderCanceledAdapter
...
import vn.techres.supplier.helper.AppUtils.formatFloatToString
 ...
 * ================================================
 */
class OrderCanceledDetailFragment : BaseBindingFragment<FragmentOrderDetailBinding>(
    FragmentOrderDetailBinding::inflate
) {
    val tagName: String = OrderCanceledDetailFragment::class.java.name
    private var adapter: DialogBillMaterialOrderCanceledAdapter? = null
    ...
    fun setAdapter() {
        adapter = DialogBillMaterialOrderCanceledAdapter(mainActivity!!.baseContext)
                ...
                override fun onNext(response: DetailOrderBillAllResponse) {
                    if (response.status == AppConfig.SUCCESS_CODE) {
                        ...
                        binding!!.totalMoney2.text =
                            Currency.formatCurrencyDecimal(dataDetailBillOrder.total_amount!!.toFloat()) + " " + getString(
                                R.string.txt_vnd
                            )
                    } else {
                        Toast.makeText(context, response.message, Toast.LENGTH_LONG).show()
