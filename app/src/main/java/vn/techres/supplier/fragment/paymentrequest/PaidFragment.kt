...
import vn.techres.supplier.adapter.paymentrequestadapter.PaidAdapter
...
class PaidFragment :
    ...
    ClickIdDebt {
    val tagName: String = PaidFragment::class.java.name
    private var adapter: PaidAdapter? = null
    ...
    private fun setAdapter() {
        adapter = mainActivity?.let { PaidAdapter() }
    ...
    private fun getAPIListPaymentRequest() {
        ...
        params.request_url =
            "/api/supplier-restaurant-debt-payment-requests?restaurant_id=-1&restaurant_brand_id=-1&branch_id=-1&status=${TechresEnum.STATUS_2}&from_date=$dateFrom&to_date=$dateTo&limit=${TechresEnum.LIMIT_100}&page=1&is_deleted=1"
